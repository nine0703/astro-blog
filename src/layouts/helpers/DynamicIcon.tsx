...
const DynamicIcon: FC<IDynamicIcon> = ({ icon, className, style, ...props }) => {
  const IconLibrary = getIconLibrary(icon);
  const Icon = IconLibrary ? IconLibrary[icon] : undefined;

  if (!Icon) {
    return <span className="text-sm">图标未找到</span>;
  }

  // Default size if none provided via style or props
  const defaultStyle = {
    fontSize: '48px',
    ...style
  };

  return <Icon className={className} style={defaultStyle} {...props} />;
};
