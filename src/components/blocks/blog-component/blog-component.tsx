            ...
            <CardContent className='space-y-3.5'>
              ...
              <div className='flex items-center justify-between'>
                ...
                >
                  ...
                  <span className='sr-only'>阅读全文：{post.title}</span>
                </Button>
              </div>
            </CardContent>
          </Card>
...
const Blog = ({ blogData = [] }: BlogProps) => {
  ...
  const categories = ['全部', ...uniqueCategories.sort()]
    ...
    <section className='py-8 sm:py-16 lg:py-24' id='categories'>
      <div className='mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:space-y-16 lg:px-8'>
        ...
        <div className='space-y-4'>
          {selectedTab === '全部' && <p className='text-sm'>行业洞察</p>}
          {selectedTab !== '全部' && (
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbLink href='#'>行业洞察</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
          ...
          <p className='text-muted-foreground text-lg md:text-xl'>
            聚焦智慧医院、医疗数据治理、患者服务和区域协同，分享可落地的建设经验。
          </p>
        </div>

        {/* Tabs and Search */}
        <Tabs defaultValue='全部' value={selectedTab} onValueChange={handleTabChange} className='gap-8 lg:gap-16'>
